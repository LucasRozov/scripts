...
function main()
	...
	sampAddChatMessage("{ff0000}Player Tools (BETA) {00ff00}Загружен! {ff0000}Версия: " ..version.. " ", 0xff0000ff)
	  ...
	  if infotextb.v or infotextp.v then
		...
		if lafk.v and not sampIsCursorActive() then
			if testCheat("la") then
				sampSendChat("Данный скрипт находиться в разработке")
			end
		end
...
function update()
	sampAddChatMessage("Версия 1.0",-1)
	sampAddChatMessage("Дата выхода: 08.01.2022",-1)
end
