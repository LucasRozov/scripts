...
local cfg = inicfg.load({
	...
	nameRank = {
		'Стажёр', 
		'Офицер тюрьмы 1 ранга', 
		'Офицер тюрьмы 2 ранга', 
		'Старший офицер Тюрьмы', 
		'Сержант', 
		'Младший надзиратель', 
		'Надзиратель', 
		'Помощник начальника', 
...
function main()
	...
	addNotify("Меню: " .. mc .. "/jlv (или Ctrl + B)\nВзаимодействие: " .. mc .. "ПКМ + Q", 5)
...
function se.onShowDialog(dialogId, style, title, button1, button2, text) -- хук диалогов
	...
	if dialogId == 235 and await['get_rank'] then
		if not text:find('Тюрьма строгого режима') and not devmode then
			lua_thread.create(function()
				wait(2000)
				addBankMessage('Вы не в организации {M}«Тюрьма Строгого Режима»')
				addBankMessage('Скрипт работает только в этой организации')
				addBankMessage('Если вы считаете, что это ошибка - напишите разработчику {M}vk.com/lucasrozov')
				addBankMessage('Скрипт отключен..')
				log('Скрипт отключен. Вы не в организации "Тюрьма Строгого Режима"')
				thisScript():unload()
			end)
			return false
		end
	...
	if string.find(title, "Успеваемость") then
		...
		elseif not mMenu.status and mMenu.timer and os.clock() - mMenu.timer < 5.00 then
			...
			local cards = string.match(text, "Сделанных патронов на складе: {FFB323}(%d+)")
			if cards then mMenu.cards.all = tonumber(cards) end
			local cards = string.match(text, "Сделанных патронов на складе: {F9FF23}(%d+)")
			if cards then mMenu.cards.today = tonumber(cards) end

			local dep = string.match(text, "Доставленных в ПД фур: {FFB323}(%d+)")
			if dep then mMenu.dep.all = tonumber(dep) end
			local dep = string.match(text, "Доставленных в ПД фур: {F9FF23}(%d+)")
...
function imgui.OnDrawFrame()
	...
	if int_bank.alpha > 0.00 then
		...
		imgui.Begin(u8'##IntMenuBank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
			...
			if TypeAction.v == 2 or TypeAction.v == 3 then
				...
				if TypeAction.v == 1 then
					if CREDIT_AWAIT == nil then
						...
						if imgui.Button(u8('Сковать в наручники ')..fa.ICON_FA_CHILD, imgui.ImVec2(250, 30)) then
							play_message(MsgDelay.v, false, {
								{ "/me {sex:снял|сняла} наручники с пояса" },
								{ "/me {sex:надел|надела} наручники на человека" },
								{ "/cuff %s", actionId }
							})
						end
						...
						if cfg.main.rank >= 3 then
							if imgui.Button(u8('Выпустить по УДО ')..fa.ICON_FA_MINUS_CIRCLE, imgui.ImVec2(250, 30)) then
								int_bank:switch()
								await['dep_minus'] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "/me {sex:открыл|открыла} на планшете раздел «Заключённые»" },
									{ "/me {sex:выбрал|выбрала} пункт «Удалить»" },
						...
						if imgui.Button(u8('Тащать за собой ')..fa.ICON_FA_CHILD, imgui.ImVec2(-1, 30)) then
							int_bank:switch()
							play_message(MsgDelay.v, false, {
								{ "/me {sex:схватил|схватила} человека за руку" },
								{ "/me {sex:потащил|потащила} человека за собой" },
								{ "/gotome %s", actionId }
							})
						end
				...
				if TypeAction.v == 3 then
					...
					if not mMenu.status then
						...
						imgui.TextColoredRGB( ("Сделано патрон на складе: " .. mc .. "%d"):format(mMenu.cards.all) )
						imgui.SameLine(180)
						imgui.TextDisabled(u8("(Сегодня: %d)"):format(mMenu.cards.today))

						imgui.TextColoredRGB( ("Доставлено фур: " .. mc .. "%d"):format(mMenu.dep.all) )
						imgui.SameLine(180)
			...
			imgui.RadioButton(u8("Меню начальника"), TypeAction, 1)
	...
	if bank.alpha > 0.00 then
		...
		if type_window.v == 3 then -- Правила
			...
			if imgui.Button(u8('Устав Тюрьмы строгого режима LV ')..fa.ICON_FA_BOOK_OPEN, imgui.ImVec2(-1, 35)) then
