	...
	local jsn_upd = "https://raw.githubusercontent.com/LucasRozov/scripts/main/jlvupd.json"
	...
	function main()
		...
		log('������ ����� � ������!', "����������")
		addNotify("����: " .. mc .. "/jlv (��� Ctrl + B)\n��������������: " .. mc .. "��� + Q", 5)
		addNotify("������� ������: " .. mc .. thisScript().version .. "\n������ ��������!", 5)
		...
		sampRegisterChatCommand('jlv', function()
			bank:switch()
		end)

		if sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))) == "Lucas_Rozov" or "jlvhelper.lua" then
			log('�� ����� ��� �����������!')
			devmode = true
		...
		while true do
			...
			if sampIsChatInputActive() and chat_calc.v then
				...
				if isCalled and string.find(input, "[0-9]+") and string.find(input, "[%+%-%*%/%^]+") then 
					...
					if result then
						local output = string.format('���������: {50AAFF}%s', answer)
						if tonumber(answer) then
							...
							output = string.format('���������: {50AAFF}%s', table.concat(result))
						end
			...
			elseif SPEAKING and not SPEAKING.dead then
				...
				local text = "������� " .. sc .. "BACKSPACE{FFFFFF}, ���� ������ �������� ���������"
	...
	function se.onServerMessage(clr, msg)
		...
		local rank = string.match(msg, "����� [A-z0-9_]+ ������� �� (%d+) �����")
		if rank ~= nil then
			rank = tonumber(rank)
			cfg.main.dateuprank = os.time()
			cfg.main.rank = rank

			msg = string.format("������������ �������� ��� ����� ���������: {EEEEEE}%s(%d)", cfg.nameRank[rank], rank)
			return { clr, msg }
		end

		if msg:find(sampGetPlayerNickname(self_id)..' ������������� � ����������� ������') then
			addNotify('�� ����� ����� � ���������\n���� ������� ����!', 5)
			return false
		end
		...
		if member and client and reason then 
			if chat['expel'].v then
				msg = string.format(tag .. '��������� %s ������ �� ����� %s{FFFFFF}. �������: %s', member, sc .. client, reason)
				return { 0x3F68D1FF, msg }
			end
			return false
		end

		local sum, nick = msg:match('%[����%] {%x+}����������� �������� %$([%p%d]+) %(%d+ ���������%) �� ������ ������ ������� ([A-z0-9_]+)')
		if sum and nick then
			sum = sum:gsub("%p", "")
			if chat['shtrafs'].v then
				msg = string.format(tag .. '����� ����� ��������� �� $%s{FFFFFF}. ������ %s{FFFFFF} ���� ������ �� �����', sc .. sum, sc .. nick:gsub('_', ' '))
				return { 0x3F68D1FF, msg }
			end
			return false
		...
		if nick and sum then 
			if chat['incazna'].v then
				msg = string.format(tag .. '��������� %s{FFFFFF} �������� ����� ����� �� %s', sc .. nick:gsub('_', ' '), sc .. sum)
				return { 0x3F68D1FF, msg }
			end
			return false
		end
		...
		if member and leader then 
			if chat['invite'].v then
				msg = string.format(tag .. '����� ���� ����� ����������� - %s{FFFFFF}, �������� ������(�) %s', sc .. member:gsub('_', ' '), sc .. leader:gsub('_', ' '))
				return { 0x3F68D1FF, msg }
			end
			return false
		end
		...
		if leader and member and reason then 
			if chat['uval'].v then
				msg = string.format(tag .. '������������ %s{FFFFFF} ������ ���������� %s{FFFFFF} �� �������: %s', sc .. leader:gsub('_', ' '), sc .. member:gsub('_', ' '), sc .. reason)
				return { 0x3F68D1FF, msg }
			end
			return false
		end
		...
		if msg:find("^����� ���������� �� Arizona Role Play!$") then
	...
	function se.onShowDialog(dialogId, style, title, button1, button2, text) -- ��� ��������
		...
		if string.find(title, "�������� ������", 1, true) and await["wbook"] then
			local nick = (actionId == nil) and "����������" or rpNick(actionId)
			lua_thread.create(function(); wait(300)
				sampSendChat("/me {sex:����|�����} �������� ������ � " .. nick)
		...
		if string.find(title, "�������", 1, true) and await["passport"] then
			local nick = (actionId == nil) and "����������" or rpNick(actionId)
			lua_thread.create(function(); wait(300)
				sampSendChat("/me {sex:����|�����} ������� � " .. nick)
				wait(MsgDelay.v * 1000)
			...
			local law = string.match(text, "�����������������: {%x+}(%d+)/%d+")
		...
		if string.find(title, "���. �����", 1, true) and await["medcard"] then
			local nick = (actionId == nil) and "����������" or rpNick(actionId)
			lua_thread.create(function(); wait(300)
				sampSendChat("/me {sex:����|�����} ����������� �������� � " .. nick)
				wait(MsgDelay.v * 1000)
			...
			sobes_check["health"] = string.find(text, "��������� ��������") ~= nil

			await["medcard"] = nil
		end

		if string.find(title, "��������", 1, true) and await["licenses"] then
			local nick = (actionId == nil) and "����������" or rpNick(actionId)
			lua_thread.create(function(); wait(300)
				sampSendChat("/me {sex:����|�����} �������� ��������� � " .. nick)
				wait(MsgDelay.v * 1000)
		...
		if await["uniform"] and text:find("�����������") then
		...
		if dialogId == 713 and isAnyAwaitExist then
			...
			elseif await['dep_minus'] then 		send(1, 5); await['dep_minus'] = nil 			-- ����� ������ � ��������
			elseif await['dep_plus'] then 		send(1, 6); await['dep_plus'] = nil 			-- ��������� �������
			--elseif await['dep_plus_10'] then 	send(1, 7); await['dep_plus_10'] = nil 			-- ������ ����������� �������
			elseif await['dep_check'] then 		send(1, 7); await['dep_check'] = nil 			-- ��������� ������� ����� ��������
			elseif await['vip_create'] then 	send(1, 8); await['vip_create'] = nil 			-- ������ ���������� ����� VIP �������
		...
		if string.find(title, "����������") and string.find(text, "������ �������") then
			...
			for line in string.gmatch(text, "[^\n]+") do
				...
				local price = string.match(line, "���� ������: {%x+}(%d+)")
				if price then info.price = tonumber(price) end

				local farm = string.match(line, "������� ��������� �� �����: {%x+}(%d+)")
				if farm then info.farm = tonumber(farm) end

				local buy_ls = string.match(line, "������� ��������� � Los Santos: {%x+}(%d+)")
				if buy_ls then info.buy_ls = tonumber(buy_ls) end

				local buy_lv = string.match(line, "������� ��������� � Las Venturas: {%x+}(%d+)")
			...
			result = result .. "\n"
			result = result .. "{FFFFFF}���������� �������: {73B461}����������� ����\n"

			result = result .. "\n"
			result = result .. ("{FFFFFF}���� ������: {73B461}%s\n"):format(sumFormat(info.price))

			result = result .. "\n"
			result = result .. ("{FFFFFF}������� ��������� �� �����: {73B461}%s\n"):format(sumFormat(info.farm))
			result = result .. ("{FFFFFF}������� ��������� � Los Santos: {73B461}%s\n"):format(sumFormat(info.buy_ls))
			result = result .. ("{FFFFFF}������� ��������� � Las Venturas: {73B461}%s"):format(sumFormat(info.buy_lv))
		...
		if dialogId == 235 and await['get_rank'] then
			if not text:find('������ �������� ������') and not devmode then
				lua_thread.create(function()
					...
					addBankMessage('�� �� � ����������� {M}������� �������� ������')
					addBankMessage('������ �������� ������ � ���� �����������')
					addBankMessage('���� �� ��������, ��� ��� ������ - �������� ������������ {M}vk.com/lucasrozov')
					addBankMessage('������ ��������..')
					log('������ ��������. �� �� � ����������� "������ �������� ������"')
					thisScript():unload()
				end)
			...
			local stat_info = string.match(text, '���������: {%x+}([^\n]+)')
		...
		if string.find(title, "������������") then
			if await['uprankdate'] then
				local d, m, Y, H, M, S = string.match(text, "��������� ���������:\n {%x+}(%d+)%.(%d+)%.(%d+) (%d+):(%d+):(%d+)")
				if d ~= nil then
					local datetime = { year = Y, month = m, day = d, hour = H, min = M, sec = S}
					addBankMessage(string.format('���� ���������� ��������� �� /jobprogress: {M}%s', os.date('%d.%m.%Y', os.time(datetime))))
		...
		if #pincode.v > 0 then
			if string.find(text, "�� ������ ����������� ���� PIN-��� � ��������", 1, true) then
				sampSendDialogResponse(dialogId, 1, nil, pincode.v)
				return false
			end

			if string.find(text, "PIN-��� ������!", 1, true) then
				text = "{FFFFFF}PIN-��� ������ {33AA33}�������������!"
				return { dialogId, style, title, button1, button2, text }
			end

			if string.find(text, "�� �� ��������� ����� PIN-���!", 1, true) then
				text = "{FFFFFF}PIN-��� ��� ��������������� ����� {AA3333}��������!\n{FFFFFF}��� ����� ������ ��� �������"
				button1 = "�������"
		...
		if await["quest"] and string.find(text, "�� ������������� ������ ������� �����?") then
	...
	function imgui.OnDrawFrame()
			...
			imgui.Begin(u8'##IntMenuBank', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
				...
				if sampIsPlayerConnected(actionId) then
					...
					if TypeAction.v == 2 and yearold < 3 then 
						imgui.CenterTextColoredRGB('{FF0000}' .. yearold .. ' �������')
					else
				...
				if bPed then
					...
					imgui.CenterTextColoredRGB(bas_skin and '�� ��������' or '��������')
				else
				...
				if TypeAction.v == 2 or TypeAction.v == 3 then
					...
					if TypeAction.v == 1 then
						if CREDIT_AWAIT == nil then
							...
							if imgui.Button(u8('������� � ��������� ')..fa.ICON_FA_CHILD, imgui.ImVec2(250, 30)) then
							...
							if cfg.main.rank >= 2 then
								...
								imgui.Hint('expelbut', u8'Test Button, dont click!')
							else
							...
							if cfg.main.rank >= 3 then
								...
								imgui.Hint('getdeptime', u8'Test Button, dont click!')
								if imgui.IsItemClicked(1) then
									await['dep_check'] = os.clock()
								...
								imgui.Hint('depositecheck3rank', u8'�������� � 3 �����')
							end
						else
					...
					if TypeAction.v == 2 then
						...
						imgui.TextColored(imgui.ImVec4(1, 1, 1, 0.3), u8'(�� �����������)')
						imgui.SameLine(275 - check_offset)
						...
						do
							...
							if imgui.Button(u8('�������'), imgui.ImVec2(norm_len / count, 30)) then
								await["passport"] = os.clock()
								play_message(MsgDelay.v, false, {
									{ "������ �����, �������� ��� �������" },
									{ "/b /showpass {my_id} + 1-2 ���������" }
								})
							end
						...
						do
							local count = 3
							local norm_len = len - (spacing * (count - 1))
							if imgui.Button(u8('������ 1'), imgui.ImVec2(norm_len / count, 30)) then
								play_message(MsgDelay.v, false, {
									{ "�������� �� �� ����� � ����� �����������? ���� ��, �� ������ ����?" }
								})
							end
							imgui.SameLine(nil, spacing)
							if imgui.Button(u8('������ 2'), imgui.ImVec2(norm_len / count, 30)) then
								play_message(MsgDelay.v, false, {
									{ "���������� �� �� ��������� ����� ����� �����������?" }
								})
							end
							imgui.SameLine(nil, spacing)
							if imgui.Button(u8('������ 3'), imgui.ImVec2(norm_len / count, 30)) then
								play_message(MsgDelay.v, false, {
									{ "������� �� ����� ����� ���� ������ ������� ��������� � ���?" }
								})
							end
						...
						do
							...
							if imgui.Button(u8('������� ')..fa.ICON_FA_USER_PLUS, imgui.ImVec2(norm_len / 2, 30)) then
								if not isPlayerOnBlacklist(sampGetPlayerNickname(actionId)) then 
									...
									if cfg.main.rank >= 9 then
										...
										play_message(MsgDelay.v, true, {
											...
											{ "/rb ������ �������������: %s[%s]", rpNick(actionId), actionId }
										})
									end
								else
									addBankMessage('�� �� ������ ������� ����� ������ �� �������!')
							...
							if imgui.Button(u8('��������� ')..fa.ICON_FA_USER_TIMES, imgui.ImVec2(norm_len / 2, 30), { 0.3, 0.1 }) then
								imgui.OpenPopup(u8("������� ����������"))
							end
							imgui.PopStyleColor(2)
						end
						...
						if imgui.BeginPopupModal(u8("������� ����������"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
							...
							if imgui.GreenButton(u8('����� ������� �������'), imgui.ImVec2(250, 30)) then
					...
					if TypeAction.v == 3 then
						...
						if not mMenu.status then
							if os.clock() - mMenu.timer < 5.00 then
								...
								imgui.PopFont()
								imgui.CenterTextColoredRGB("{AAAAAA}�� ������� �������� ����������..\n{AAAAAA}��������� ��� ��� ��� ���������!")
								imgui.NewLine()
							...
							imgui.TextColoredRGB( ("������� �� ������: " .. mc .. "%d"):format(mMenu.time.all) )
							imgui.SameLine(180)
							imgui.TextDisabled(u8("(�������: %d)"):format(mMenu.time.today))

							imgui.TextColoredRGB( ("������� ������ �� ������: " .. mc .. "%d"):format(mMenu.cards.all) )
							imgui.SameLine(180)
							imgui.TextDisabled(u8("(�������: %d)"):format(mMenu.cards.today))

							imgui.TextColoredRGB( ("���������� ���: " .. mc .. "%d"):format(mMenu.dep.all) )
							imgui.SameLine(180)
							imgui.TextDisabled(u8("(�������: %d)"):format(mMenu.dep.today))

							imgui.TextColoredRGB( ("{60FF70}���(�) �������(�) %s"):format(mMenu.last_up and stringToLower(getTimeAfter(mMenu.last_up)) or "���������� �����") )
							if mMenu.last_up then
								imgui.Hint("rankupdate", u8(os.date("%d.%m.%Y � %H:%M", mMenu.last_up)))
							end
							imgui.NewLine()

							local norm_len = ws.x - (5 * 2) - (imgui.GetStyle().WindowPadding.x * 2)
							if imgui.MainButton(u8"������ ����", imgui.ImVec2(norm_len / 3, 20)) then
							...
							local rank_name = u8(cfg.nameRank[giverank.v] or "����������")
							if imgui.MainButton(u8("�������� �� [ %s ]"):format(rank_name), imgui.ImVec2(-1, 25)) then
								int_bank:switch()
								play_message(MsgDelay.v, true, {
									{ "/me {sex:������|�������} �� ������� ���" },
									{ "/me {sex:�������|��������} ��� � {sex:�����|�����} � ������ �����������" },
				...
				if cfg.main.rank > 4 then
					if imgui.RadioButton(u8("�������������"), TypeAction, 2) then
				...
				if cfg.main.rank >= 9 then
					if imgui.RadioButton(u8("���� ����������"), TypeAction, 3) then
		...
		if bank.alpha > 0.00 then
			...
			imgui.BeginGroup()
				...
				imgui.TextColored(imgui.GetStyle().Colors[imgui.Col.ButtonHovered], fa.ICON_FA_UNIVERSITY.. ' JLV Helper')
				local len_title = imgui.CalcTextSize(fa.ICON_FA_UNIVERSITY.. ' JLV Helper').x
				imgui.PopFont()
			...
			if type_window.v == 1 then -- �����
				if #cfg.Binds_Name > 0 then
					...
					imgui.Hint('binderCtrlB', u8'����������� Ctrl + B ���\n�������� �������� ����� ����')
					imgui.Separator()
			...
			if type_window.v == 3 then -- �������
				...
				if imgui.Button(u8('����� ������ �������� ������ LV ')..fa.ICON_FA_BOOK_OPEN, imgui.ImVec2(-1, 35)) then
			...
			if type_window.v == 5 then -- ���������
				...
				if imgui.Checkbox(u8'����-�������', auto_stick) then
					cfg.main.auto_stick = auto_stick.v 
					inicfg.save(cfg, 'Bank_Config.ini')
				end
				imgui.SameLine()
				imgui.TextDisabled('(?)')
				imgui.Hint('autostick', u8'�� ������������� ������� �������, ��� ������ ������ �� ������\n������� ������ ������ ������ ���� ������� ����������� (/setspawn)')
				...
				if string.find(cfg.main.pincode, "%S") == nil then
					imgui.SameLine(10)
					imgui.TextDisabled(u8"PIN-��� �� �����")
				end
				
				if imgui.InputText(u8"��� ���������##USERNAME", username) then
					cfg.main.username = u8:decode(tostring(username.v))
				end
				imgui.SameLine()
				imgui.TextDisabled('(?)')
				imgui.Hint('usernamehint', u8'��� ��������� ������������ � ����������.\n�������� �� ������ ������ ���� ��� �� �������')
				...
				imgui.SameLine()
				imgui.TextDisabled("(?)")
				imgui.Hint("handwritehelp", u8"������� CTRL + ���, ����� ������ �������")
				...
				if imgui.Button(u8('�������� ������� ')..fa.ICON_FA_PUZZLE_PIECE, imgui.ImVec2(150, 64)) then
					type_window.v = 6
				end
				...
				imgui.BeginGroup()
					if imgui.RedButton(u8'������� Bank-Helper', imgui.ImVec2(-1, 30)) then
					...
					if imgui.Button(u8('��������� ���������� ')..fa.ICON_FA_DOWNLOAD, imgui.ImVec2(-1, 30)) then
						lua_thread.create(autoupdate, jsn_upd); checkData()
					end
				imgui.EndGroup()

				if imgui.GreenButton(u8'������ ���������', imgui.ImVec2(-1, 20)) then
			...
			if type_window.v == 6 then 
				...
				for i, info in ipairs(Repository) do
					if imgui.CollapsingHeader(u8(info.name)) then
						imgui.TextColoredRGB(mc .. "��� �����: {SSSSSS}" .. info.file)
						...
						if #info.cmds > 1 then
							imgui.TextColoredRGB(mc .. "������� �������: {SSSSSS}" .. table.concat(info.cmds, ", "))
						elseif #info.cmds == 1 then
							imgui.TextColoredRGB(mc .. "������� �������: {SSSSSS}" .. info.cmds[1])
						else
						...
						if #missing > 0 then
							imgui.DisableButton(u8("���������� ") .. fa.ICON_FA_DOWNLOAD .. "##Repo" .. i, imgui.ImVec2(150, 20))
						...
						elseif not doesFileExist(path) then
							if imgui.GreenButton(u8("���������� ") .. fa.ICON_FA_DOWNLOAD .. "##Repo" .. i, imgui.ImVec2(150, 20)) then
								downloadUrlToFile(info.url, path, function (id, status, p1, p2)
									if status == STATUSEX_ENDDOWNLOAD then
										addBankMessage(string.format("������ �{M}%s{W}� ������� ��������!", info.name))
										script.load(path)
									end
								end)
							end
						else
							imgui.DisableButton(u8("����������� ") .. fa.ICON_FA_CHECK_CIRCLE .. "##Repo" .. i, imgui.ImVec2(150, 20))
							imgui.SameLine()
							if imgui.RedButton(u8("������� ") .. fa.ICON_FA_TRASH .. "##RepoDel" .. i, imgui.ImVec2(90, 20)) then
		...
		if ustav_window.alpha > 0.00 then
			...
			imgui.Begin(u8'����� ������������ �����', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar)
				...
				local results = 0
				for line in io.lines(getWorkingDirectory() .. "\\BHelper\\����� ��.txt") do
					if #line == 0 then line = "\n" end
				...
				if #search_ustav.v > 0 and results == 0 then
					...
					imgui.CenterTextColoredRGB('{666666}������, ������ �� ������� :(')
				end
				...
				if v.active then
					...
					imgui.Begin(u8'##notifycard'..k, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoFocusOnAppearing + imgui.WindowFlags.NoBringToFrontOnFocus)
						...
						imgui.BeginGroup()
							...
							imgui.PushFont(font[15])
							imgui.TextColoredRGB(mc .. 'JLV Helper')
							imgui.PopFont()
			...
			imgui.Begin(u8("����������"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoBringToFrontOnFocus + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.NoTitleBar)
				...
				imgui.BeginGroup()
					...
					if distBetweenKassa < 10 then
						...
						imgui.Text(fa.ICON_FA_CHECK_CIRCLE);    imgui.SameLine(30); imgui.TextColoredRGB(mc..'��������� ���������: {SSSSSS}'    ..tostring(kassa.info.dep))
						imgui.Text(fa.ICON_FA_HANDSHAKE);       imgui.SameLine(30); imgui.TextColoredRGB(mc..'��������� ��������: {SSSSSS}'     ..tostring(kassa.info.credit))
						imgui.Text(fa.ICON_FA_CREDIT_CARD);     imgui.SameLine(30); imgui.TextColoredRGB(mc..'������ ����: {SSSSSS}'            ..tostring(kassa.info.card))
						imgui.Text(fa.ICON_FA_TICKET_ALT);     	imgui.SameLine(30); imgui.TextColoredRGB(mc..'������ VIP-����: {SSSSSS}'        ..tostring(kassa.info.vip))
						imgui.Text(fa.ICON_FA_RECYCLE);         imgui.SameLine(30); imgui.TextColoredRGB(mc..'������������� ����: {SSSSSS}'     ..tostring(kassa.info.recard))
						imgui.Text(fa.ICON_FA_PLUS_CIRCLE);     imgui.SameLine(30); imgui.TextColoredRGB(mc..'������� ���. ������: {SSSSSS}'    ..tostring(kassa.info.addcard))
						imgui.Text(fa.ICON_FA_PIGGY_BANK);      imgui.SameLine(30); imgui.TextColoredRGB(mc..'����������: {SSSSSS}$'            ..sumFormat(kassa.money))
					else
	...
	function system_credit()
		if imgui.BeginPopupModal(u8("������� ������������"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
			...
			credits:close()
			imgui.Separator()
			imgui.CenterTextColoredRGB('{464646}> ������������� ���� <')
	...
	function binder()
		if imgui.BeginPopupModal(u8("��������������/�������� �����"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
			imgui.BeginChild("##EditBinder", imgui.ImVec2(500, 355), true)
				imgui.PushItemWidth(150)
				imgui.InputFloat(u8("�������� � ��������"), binder_delay, 0.1, 1.0, 2); imgui.SameLine(); imgui.TextDisabled('(?)')
				imgui.Hint('maxdelaybinder', u8('�� ������ 60 ������!'))
				imgui.PopItemWidth()
		...
		if cmd:match('^/premium %d+ %d+ %d+') and cfg.main.rank >= 9 then
			...
			if rank then
				...
				if cfg.nameRank[rank] ~= nil and LAST_RANK_PREMIUM ~= cfg.nameRank[rank] then
					...
					sampSendChat('/rb ������ ��� ����������� �� ��������� ' .. LAST_RANK_PREMIUM)
					sampSendChat(cmd)
					return false
				end
			end
	...
	function helpCommands()
		if imgui.BeginPopupModal(u8("��� ������� �������"), _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar) then
			...
			imgui.BeginGroup()
				imgui.TextColoredRGB(mc..'/bank{SSSSSS} - �������� ���� ������� (���� Ctrl + B)')
				imgui.TextColoredRGB(mc..'/ustav{SSSSSS} - ����� �� � ��������� ����')
				imgui.TextColoredRGB(mc..'/ro [text]{SSSSSS} - �� ��� ����������')
				imgui.TextColoredRGB(mc..'/rbo [text]{SSSSSS} - ����� ��� ����������')
				...
				imgui.TextColoredRGB(mc..'/getprize{SSSSSS} - ��������� ����� � ������� ���. �� ����������')
				imgui.TextColoredRGB(mc..'/jp{SSSSSS} - ������������� /jobprogress')
				imgui.TextColoredRGB(mc..'/cjp{SSSSSS} - ������������� /checkjobprogress')
			imgui.EndGroup()
			imgui.Spacing()
			imgui.TextColoredRGB(sc..'��������� ������:')
			imgui.SetCursorPosX(20)
			imgui.BeginGroup()
				...
				imgui.TextColoredRGB(mc..'��� + G{SSSSSS} - ������� �� ����� � �������� ' .. cfg.main.expelReason)
	...
	changelog = {
		...
		[28] = {
			...
			log = {
				"� ���������� ��������� ������� ��������� ����� ��������� �����. ����-��� � ��� �������� ������ 3 ��������� ����� �� �����, �� �� ���������, ��� ���� ������ ����� �����������",
				"������� �������������� ������ �� ����������� � ����� ������� ����������� ������ (��������� ����������� �������� � ���������������� ������ ������)",
				"��������� ������� /jp � /cjp (����� �������e �������� ������ /jobprogress � /checkjobprogress)"
			},
			patches = {
				show = false,
				info = {
	...
	{006AC2}5-6 ���� {SSSSSS}- ��� �����, ����� �������� ������ ��, ��� �� ������� ������
	{006AC2}6-7 ���� {SSSSSS}- ��� �����, ����� �������� ������ ��, ��� �� ������� ������
	{006AC2}7-8 ���� {SSSSSS}- 100 ������
	{006AC2}8-9 ���� {SSSSSS}- 200 ������
	]]
